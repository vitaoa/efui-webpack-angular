...
+ function ($) {
    ...
    $.fn.squareDraw = function (options) {
        var options = $.extend({},$.fn.squareDraw.defaults,options);
        var $this = $(this);        //响应事件对象

        lottery.init(options.objId);

        $(options.eventObj).click(function () {
        ...
        function roll(){
            ...
            if (lottery.times > lottery.cycle+10 && lottery.prize==lottery.index) {
                ...
                if($.inArray(_currentPrize, options.hitprize)>=0){
                    $('.getWinning').show();
    ...
    // 转盘抽奖
    $.fn.rotaryDraw = function (options) {
